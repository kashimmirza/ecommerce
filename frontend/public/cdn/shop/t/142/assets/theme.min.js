            ...
            return m.prototype = Object.assign({}, m.prototype, {
                    ...
                    if (this.currentVariant && this.canMediaGroup) {
                        ...
                        if (n = u.find(a), o = f.find(a), 0 != n.length && r != this.groupValue)
                            if (this.groupValue = r, u.find("[data-main-slide]").addClass("is--media-hide"), n.removeClass("is--media-hide"), f.find(".t4s-carousel__nav-item").addClass("is--media-hide"), o.removeClass("is--media-hide"), u.hasClass("flickityt4s-enabled")) {
                                u.trigger("update.flickityt4s");
                                var p = t.featured_media ? u.find(`[data-media-id="${t.featured_media.id}"]:visible`).index() : 0;
                                u.flickityt4s("selectCell", Math.max(p, 0), !1, !1)
                ...
                _updateSwatchSelector: function(t, n, o, s) {
                    ...
                    if (o.find(".is--selected").removeClass("is--selected"), o.find("[data-current-value]").html(""), $ = this.productOptions[0].values, m.find("[data-current-value]").html(w), m.find(e).eq($.indexOf(w)).addClass("is--selected"), g[0] && (C = this.productOptions[1].values, g.find("[data-current-value]").html(_), g.find(e).eq(C.indexOf(_)).addClass("is--selected")), v[0] && (k = this.productOptions[2].values, v.find("[data-current-value]").html(T), v.find(e).eq(k.indexOf(T)).addClass("is--selected")), s) switch (y) {
                        case 3:
                            for (1 == k.length ? (r = m, d = $, l = "option3", c = T, u = "option1", h = 3) : (r = v, d = k, l = "option1", c = w, u = "option3"), S = (b = this._getVariantFromOptionIndex(h, t)).length, r.find(e).addClass(a), g.find(e).addClass(a), p = 0; p < S; p++)(f = b[p])[l] == c && (f.available ? (g.find(e).eq(C.indexOf(f.option2)).removeClass(a), f.option2 == _ && r.find(e).eq(d.indexOf(f[u])).removeClass(a)) : (g.find(e).eq(C.indexOf(f.option2)).removeClass(i), f.option2 == _ && r.find(e).eq(d.indexOf(f[u])).removeClass(i)));
                            break;
                        case 2:
            ...
            return o.prototype = Object.assign({}, o.prototype, {
                ...
                _orderDelivery: function() {
                    ...
                    if (0 != e.length) {
                        ...
                        if (n && g) try {
                            y = (v = dayjs.tz(v, i)).format("HHmmss")
                        } catch (t) {
                            console.log("Timezone error: " + i)
                        }
                        if (parseInt(y) >= parseInt(b) && (v = v.add(1, "day"), f = f.add(1, "day"), h = h.add(1, "day")), "2" == a.mode) {
                            for (; c.indexOf(r[f.format("d")]) > -1 || u.indexOf(f.format("DD/MM")) > -1;) f = f.add(1, "day");
                            for (; p < d;) p++, f = f.add(1, "day"), (c.indexOf(r[f.format("d")]) > -1 || u.indexOf(f.format("DD/MM")) > -1) && p--;
                            for (; c.indexOf(r[h.format("d")]) > -1;) h = h.add(1, "day");
                            for (; m < l;) m++, h = h.add(1, "day"), c.indexOf(r[h.format("d")]) > -1 && m--
                        } else {
                            for (f = f.add(d, "day"); c.indexOf(r[f.format("d")]) > -1 || u.indexOf(f.format("DD/MM")) > -1;) f = f.add(1, "day");
                            for (h = h.add(l, "day"); c.indexOf(r[h.format("d")]) > -1;) h = h.add(1, "day")
                        }
                        w = V(w), _ = V(_);
                        var T = parseInt(f.format("D")),
                            $ = T + A(T),
                            C = _[parseInt(f.format("M")) - 1],
                            k = w[parseInt(f.format("d"))],
                            I = parseInt(h.format("D")),
                            x = I + A(I),
                            P = _[parseInt(h.format("M")) - 1],
                            M = w[parseInt(h.format("d"))];
                        if (e.find("[data-start-delivery]").html(f.format(o).replace("t44", k).replace("t45", $).replace("t46", C)), e.find("[data-end-delivery]").html(h.format(o).replace("t44", M).replace("t45", x).replace("t46", P)), "19041994" != s) {
                            var O = e.find("[data-hour-delivery]");
                            O.countdown(v.format("YYYY-MM-DD " + s), {
                                elapse: !0
                            }).on("update.countdown", function(i) {
                                if (i.elapsed) e.hide();
                                else {
                                    var a = 24 * i.offset.totalDays + i.offset.hours;
                                    O.html(i.strftime(t.trim(O.html().replace("[totalHours]", a)))).show()
                                }
                            })
                        }
                        e.show()
        ...
        y = function() {
            ...
            function v(i, n, c) {
                ...
                a.find("[data-action-atc][data-action-delay]").on(l, function(i) {
                    !t(this)[0].hasAttribute("data-action-delay") || e.width() > 767 || (i.preventDefault(), i.stopPropagation(), _ = !0, S.slideDown({
                        start: function() {
                            t(this).css({
                                display: "flex"
                    ...
                    !_ || e.width() > 767 || (i.preventDefault(), _ = !1, S.slideUp({
                        start: function() {
            ...
            return v.prototype = Object.assign({}, v.prototype, {
                ...
                _stickyAddToCartToggle: function(n) {
                    var f = e.scrollTop(),
                        p = parseInt(f + c) + Math.max(t("#t4s-footer").outerHeight() / 2, 120);
                    u = i.height(), s < f && p !== u && p < u ? (a.addClass(d.isShow), o.addClass(d.isActive), y()) : (p === u || p > u || s > f && n) && (a.removeClass(d.isShow), o.removeClass(d.isActive), a.find("[data-dropdown-open].is--clicked").click(), r.trigger(l))
                }
            }), v
        }();
